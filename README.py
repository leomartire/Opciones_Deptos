...
# 3. CSS COMPLETO (Branding 2026 + Ajuste de Botones y Márgenes)
st.markdown("""
    <style>
    ...
    /* Ajuste de margen para evitar cortes en el banner superior */
    ...
    /* Botón VER (Pequeño, Minimalista y Proporcional) */
    .stButton>button {
    ...
    /* Contenedor específico para el botón VOLVER (Más grande y accesible) */
    .boton-volver-container .stButton>button {
...
# 4. CARGA DE DATOS (Con TTL para actualizaciones dinámicas)
    ...
    if st.session_state.opcion_actual == "HOME":
        ...
        if df_home is not None:
            ...
            for index, row in df_home.iterrows():
                ...
                # Filtrado de encabezados técnicos y filas vacías
                if (not val_raw or val_raw.upper() in ["UNIDAD", "HOME"] or 
                    val_raw.isdigit() or val_raw in unidades_vistas):
                    continue
                
                unidades_vistas.add(val_raw)
                
                # Ajuste de columnas para alineación perfecta del botón sutil
                ...
                with col3:
                    ...
                    st.markdown(f"<p class='texto-base' style='text-align:right; line-height:24px;'>{val_contacto}</p>", unsafe_allow_html=True)
        ...
        # Contenedor especial para el botón de volver
        st.markdown('<div class="boton-volver-container">', unsafe_allow_html=True)
        if st.button("← VOLVER AL PANEL", key="btn_back_final"):
            st.session_state.opcion_actual = "HOME"
            st.rerun()
